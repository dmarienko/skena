...
**Architecture:** one view of "a kernel" (`KernelLike`) resolved by `kernelById(canvas, id)` from records first, then nodes; every host and MCP consumer of `KernelNode` reads through it and mutates the returned live object (record or node) for the `kernelId` write-back. Records are webview-owned for existence, colour and section binding (saved with the canvas like sections) and host-authoritative for the live `kernelId` (merged on save). The rail's picker lists records with a live LED, then legacy nodes, then *New kernel…* (the existing host QuickPick, answering with a record instead of a node) and *Remove kernel…*. Run-with-upstream on a section-bound chain orders the edge-connected code cells by `(y, x)`.
...
- [ ] **Events** — `App.tsx`: `case 'kernelAdded'` → `window.dispatchEvent(new CustomEvent('skena:kernelAdded', { detail: msg }))`; `case 'kernelRemoved'` likewise. `CanvasView`: on `kernelAdded` → `pushHistory(); commitKernels([...kernels, kernel]); commitLanes(lanes.map(l => l.id === sectionId ? { ...l, kernelId: kernel.id } : l));`. On `kernelRemoved` → `pushHistory(); commitKernels(kernels.filter(k => k.id !== id)); commitLanes(lanes.map(l => l.kernelId === id ? { ...l, kernelId: undefined } : l));`. On `runOutput` (existing handler ~2717): if `msg.kernelNodeId` names a record and `msg.kernelId` differs → `commitKernels` with the updated `kernelId` (so the LED and the saved file follow the live kernel).
