...
Applied wherever a node is written: the webview (one hook watching node geometry, so every placement
path is covered without touching each), every MCP write (`canvas_add_node`, `canvas_update_node`,
`canvas_layout`, `canvas_pin_output`, the run's output cell), and the host's output-cell creation.
...
2. title — `writing-mode: vertical-rl; transform: rotate(180deg)` so it reads bottom → top (R2);
   system font 600 10.5px; `S1:` in the section colour, the title in `text1` for the current section and `text2`
   otherwise; falls back to the creation datetime `YYYY-MM-DD HH:MM`
...
overlapping; the tooltip still names them. Every segment has a tooltip `S2: <title> · <created, when titled> · <kernel name> (· folded)
· double-click the title to rename`, which is how short segments stay readable.
...
| title (double-click) | `TitleEditor` popover beside the segment: input, Enter or blur saves, Esc cancels; empty → title cleared |
...
and stops. One run per section per document at a time: a second `▶` while a run is in flight is answered with an
...
Every call site uses it: the host's run, complete, inspect and interrupt handlers and the MCP
`canvas_run_cell` (one-shot form), and the webview `CodeNode` (one resolver per store snapshot,
lanes from `LanesContext`). `resolveKernelCellsInCanvas(kernelId, c)` — every code cell that
resolves to that kernel — replaces the edge-only `resolveKernelCells` in the two restart/refresh
sweeps, so they reach section-bound cells. `resolveUpstreamChain` is unchanged: a section-bound
...
`constrain`; measured at +359px above y = 0 on letterboxed content), so the two fit paths and the
controls' fit button go through `fitClamped` (the `fitView` prop is gone): fit, then clamp the landed viewport. Deleting the first
section re-parks the new topmost lane at `y = 0` (`parkFirstLaneAtOrigin`), so the camera has no band
above the lane boundary; the surviving content stays where it was (compaction is the layout engine's
job). Deleted:
...
| `src/extension/mcp/server.ts` | `canvas_run_cell` → `resolveCellKernel`; `applyLaneGrowth` on every write; `canvas_list` / `canvas_read` show sections; output cell via `outputCellGeom` |
| `src/webview/canvas/LanesContext.ts` | new: `metadata.sections` for node components |
| `src/webview/theme.ts` | new: writes the `--sk-*` variables from the VS Code theme class |
| `src/webview/rail/useLaneGrowth.ts` | new: the growth hook |
...
- `outputCellGeom`, `memberCodeCellsInRunOrder`, `parkFirstLaneAtOrigin`, `clampCameraToOrigin`.
...
Known, pre-existing, not fixed here: 26 sites in `CanvasView.tsx` use `window.innerWidth` /
