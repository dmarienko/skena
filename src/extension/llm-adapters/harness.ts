...
export class HarnessAdapter implements ILLMClient {
  ...
  async chat(
    ...
    const prompt = formatPrompt(history);
    ...
    const args = [
      ...
      '--strict-mcp-config',                    // - skena MCP only; ignore the user's global MCP servers
    ];
  ...
  private spawnAndStream(
    ...
    child.stdout!.on('data', (chunk: Buffer) => {
      ...
      while ((nl = buf.indexOf('\n')) >= 0) {
        ...
        switch (ev['type']) {
          case 'system':
            if (ev['subtype'] === 'init') {
              this.log(`init: apiKeySource=${ev['apiKeySource']} model=${ev['model']} permissionMode=${ev['permissionMode']}`);
            }
            break;
          case 'assistant': {
          ...
          case 'result':
            ...
            this.log(`result: is_error=${isError} cost_usd=${ev['total_cost_usd']}`);
            break;
        }
      }
    });
    ...
    child.on('close', (code) => {
      ...
      // - fallback: tool-only turn with no streamed assistant text → surface final result
