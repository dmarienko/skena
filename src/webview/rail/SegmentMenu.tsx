 ...
 */
export function SegmentMenu({ anchor, folded, onFold, onRun, onKernel, onRename, onDelete, onClose }: {
  ...
  // - keep the box on screen: five rows plus the separator
  const top = Math.max(4, Math.min(anchor.y + 4, window.innerHeight - 6 * ROW_PX - 8));
  ...
  return createPortal(
    <div ref={ref} className="nodrag" onContextMenu={e => e.preventDefault()}
      ...
      {/* - these two hand over to another popover: closing here would take that popover down with the menu */}
